...
<script type="text/javascript">
	$(function () { 
	   	$("#add").click(function(){
			dialogModal.editPageDialog({
				url: "${ctx}/platform/permissions/sysres/edit.do?parentid="+$("#queryParentid").val(),
				editLayout:"md"
			});
	   	});
	   	$("#edit").click(function(){
			...
			dialogModal.editPageDialog({
				url: "${ctx}/platform/permissions/sysres/edit.do?id="+ids,
				editLayout:"md"
			});
	   	});
		$("#del").click(function(){
	   	...
	   	$("#refresh").click(function(){
	   		var zTree = $.fn.zTree.getZTreeObj("sysResTree");
			zTree.refresh();
			//设置隐藏域parentid分页用
			$("#queryParentid").val("");
			//重新加载表格
	...
	function loadTree(){
		var setting = {
			...
			callback: {
				onClick: sysResTreeClick
			}
		};
		$.ajax({
            type: "post",
            url: "${ctx}/platform/permissions/sysres/queryTreeJson.do",
            async:false,
            success: function(data){
            	var ret=eval("("+data+")");
	         	$(document).ready(function(){
					$.fn.zTree.init($("#sysResTree"), setting, ret);
				});
            }
        });
	};
	function sysResTreeClick(e,treeId, treeNode) {
		//改成单击展开事件
		var zTree = $.fn.zTree.getZTreeObj("sysResTree");
...
<body>
	<div class="col-sm-3" style="">
		<div id="sysResTree" class="ztree"></div>
	</div>
		...
		<table id="table" data-toggle="table" data-url="${ctx }/platform/permissions/sysres/queryAllJson.do"
			...
			<thead>
				<tr>
					...
					<th data-field="permission" data-sortable="true">权限标识</th>
				</tr>
			</thead>
		</table>
	</div>
