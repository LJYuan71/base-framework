...
<script type="text/javascript">
	$(function () { 
	   	$("#add").click(function(){
	   		window.location.href="${ctx}/platform/permissions/classifyinfo/edit.do";
	   	});
	   	$("#edit").click(function(){
			var $table = $('#table');
			...
			window.location.href="${ctx}/platform/permissions/classifyinfo/edit.do?id="+ids;
	   	});
		$("#del").click(function(){
			var $table = $('#table');
	   	 	var ids = $.map($table.bootstrapTable('getSelections'), function (row) {
						...
						success : function(res) {
							...
							if(json=="success"){
								Modal.timealert({msg: '删除成功！',time: '2000'}).on( function () {
									window.location.href="${ctx}/platform/permissions/classifyinfo/queryAll.do";
								});
							}else{
	...
	window.actionEvents = {
		'click .get' : function(e, value, row, index) {
			window.location.href="${ctx}/platform/permissions/classifyinfo/get.do?id="+row.id;
		}
	};
	//table设置查询参数
