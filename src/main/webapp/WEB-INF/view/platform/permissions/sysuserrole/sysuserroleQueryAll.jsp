...
<html lang="zh-CN">
<head>
<title>用户角色映射表管理页面</title>
</head>
<script type="text/javascript">
	$(function () { 
	   	$("#add").click(function(){
	   		window.location.href="${ctx}/platform/permissions/sysuserrole/edit.do";
	   	});
	   	$("#edit").click(function(){
			var $table = $('#table');
			...
			window.location.href="${ctx}/platform/permissions/sysuserrole/edit.do?id="+ids;
	   	});
		$("#del").click(function(){
			var $table = $('#table');
	   	 	var ids = $.map($table.bootstrapTable('getSelections'), function (row) {
			...
			Modal.confirm({msg:"删除后数据不可恢复，是否确定删除？"}).on( function (e) {
			  	if(e){
			  		...
			  		$.ajax({
						type : "post",
						url : '${ctx}/platform/permissions/sysuserrole/del.do?ids='+ids,
						success : function(res) {
							var json = eval("(" + res + ")");
							if(json=="success"){
								Modal.timealert({msg: '删除成功！',time: '2000'}).on( function () {
									window.location.href="${ctx}/platform/permissions/sysuserrole/queryAll.do";
								});
							}else{
	...
	window.actionEvents = {
		'click .get' : function(e, value, row, index) {
			window.location.href="${ctx}/platform/permissions/sysuserrole/get.do?id="+row.id;
		}
	};
	//table设置查询参数
...
<body>
	...
	<div id="toolbar" style="margin-left:10px;">
	    <button type="button" class="btn btn-success" id="add" >新增用户角色映射表</button>
	    <button type="button" class="btn btn-success" id="edit" >编辑用户角色映射表</button>
	    <button type="button" class="btn btn-success" id="del" >删除用户角色映射表</button>
	     <button type="button" class="btn btn-success" id="find" >搜索</button>
	</div>
	<table id="table" data-toggle="table" data-url="${ctx }/platform/permissions/sysuserrole/queryAllJson.do"
