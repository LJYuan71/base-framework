...
<script type="text/javascript">
	$(function () { 
	   	$("#add").click(function(){
			dialogModal.editPageDialog({
				url: "${ctx}/secrecy/basic/classifyinfo/edit.do?parentid="+$("#queryParentid").val(),
				editLayout:"md"
			})
	   	});
	   	$("#edit").click(function(){
			...
			dialogModal.editPageDialog({
				url: "${ctx}/secrecy/basic/classifyinfo/edit.do?id="+ids,
				editLayout:"md"
			})
	   	});
		$("#del").click(function(){
			...
			Modal.confirm({msg:"删除后数据不可恢复，是否确定删除？"}).on( function (e) {
			  	if(e){
			  		...
			  		$.ajax({
						type : "post",
						url : '${ctx}/secrecy/basic/classifyinfo/del.do?ids='+ids,
	...
	function loadTree(){
		...
		$.ajax({
            type: "post",
            url: "${ctx}/secrecy/basic/classifyinfo/queryTreeJson.do",
            async:false,
...
<body>
	...
	<div class="col-sm-8" >
		...
		<shiro:hasPermission name="secrecy:classifyinfo:add">
	    	<button type="button" class="btn btn-success" id="add" >新增分类信息</button>
	    </shiro:hasPermission>
	    <shiro:hasPermission name="secrecy:classifyinfo:edit">
	    	<button type="button" class="btn btn-success" id="edit" >编辑分类信息</button>
	    </shiro:hasPermission>
	    <shiro:hasPermission name="secrecy:classifyinfo:del">
		...
		<table id="table" data-toggle="table" data-url="${ctx}/secrecy/basic/classifyinfo/queryAllJson.do"
