...
import com.platform.common.utils.FtpClientUtils;
import com.platform.core.query.Result;
		...
		if (contentType.indexOf("multipart/form-data") >= 0) {
			...
			while (fileItems.hasNext()) {
				...
				// 域的名称或者以默认的头像域名称的部分“__avatar”打头(默认的头像域名称：__avatar1,2,3...，可在插件配置参数中自定义，参数名：avatar_field_names)
					...
					String username=FileUtil.readFromProperties(path, "ftp.username");
					String password=FileUtil.readFromProperties(path, "ftp.password");
					String ip=FileUtil.readFromProperties(path, "ftp.ip");
					int port=Integer.parseInt(FileUtil.readFromProperties(path, "ftp.port"));
					
					FtpClientUtils ftpClientUtils = new FtpClientUtils(username,password, ip, port);
					try {
						ftpClientUtils.connect();
						ftpClientUtils.uploadFile(inputStream, virtualPath);
					} catch (IOException e) {
						e.printStackTrace();
					}
					inputStream.close();
